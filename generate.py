...
parser_.add_argument('--test_file',dest='test',default=os.path.join(os.getcwd(),'horse2zebra'),help='test path')
...
def inference(reuse=False):
    model=CycleGan()
    i=np.random.randint(0,min(len(C),len(D)))
    ...
    cyc_A=model.Generator(gen_B,'G_BtoA',dim=model.g_dim,reuse=True)
    cyc_B=model.Generator(gen_A,'G_AtoB',dim=model.g_dim,reuse=True)
