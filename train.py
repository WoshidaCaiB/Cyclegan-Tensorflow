...
parser_=argparse.ArgumentParser(description='train')
parser_.add_argument('--model_file',dest='model',default=os.path.join(os.getcwd(),'CycleGan.ckpt'),help='model path')
parser_.add_argument('--train_file',dest='train',default=os.path.join(os.getcwd(),'horse2zebra'),help='test path')
param=parser_.parse_args()

A=read_img(os.path.join(param.train,'trainA'))
B=read_img(os.path.join(param.train,'trainB'))

def train(a,b,epoch_num=200):
    ...
    with tf.Session() as sess:
        ...
        saver.save(sess,param.model_file)
